    ...
    <div id=app>
        ...
        if (isset($_POST['account']) && isset($_POST['password']) && isset($_POST['nickname'])) {
            ...
            if ($accCheck && $pwdCheck && $nicknameCheck) {
                $sth = $dbh->prepare('SELECT account FROM user_authority WHERE account = ?');
                $sth->execute(array($_POST['account']));
                if ($sth->rowCount() == 0) {
                    $sth2 = $dbh->prepare('INSERT INTO user_authority (account, pwd, nickname, authority) VALUES (?, ?, ?, ?)');
                    $idTemp = $dbh->query('SELECT COUNT(id) FROM user_authority');
                    $idTemp_result = $idTemp->fetch();
                    $count = $idTemp_result[0];
                    $authority = $count + 200001;
                    $sth2->execute(array($_POST['account'], $_POST['password'], $_POST['nickname'], $authority));

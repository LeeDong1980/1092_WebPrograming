...
<body>
    <div id=app>
        ...
        if (isset($_POST['nickname']) && isset($_POST['password'])) {
            $sth = $dbh->prepare('SELECT account FROM user WHERE account = ? and pwd = ?');
            ...
            if ($sth->rowCount() == 1) {
                if ($_POST['newpwd1'] != '' && $_POST['newpwd2'] != '') {
                    if ($_POST['newpwd1'] == $_POST['newpwd2']) {
                        ...
                        if ($pwdCheck) {
                            $sth2 =  $dbh->prepare('UPDATE user SET nickname = ?, pwd = ? WHERE account = ?');
        ...
        <p align=center><a href="HW04.php">回到主頁面</a></p>
    </div>
</body>
